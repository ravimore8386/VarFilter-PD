...
#USAGE: perl 24_stopgain_control_cases.pl GF11 /mnt/b2/home4/arc/rm975/wgs/pedigree/GF18/variant_prio/snp/DiagnoVarFilter/dignosis /mnt/b2/home4/arc/rm975/wgs/pedigree/GF11/$samplepath/F03.snp.txt
...
open FILE, "$samplepath/input_annovar_annotated_txt_files/$item.snp.txt" or die "cannot open file"; #search contorl file
     while (my $line=<FILE>) {
     chomp($line);
     my($a1,$a2) = split /\t/, $line;
     $cont = "$a1:$a2";
...
open(my $outfile, ">","$samplepath/processed_files/fam.$famname.stopgain.cases.con") or die "cannot open file fam.$famname.stopgain.cases.con";
...
open FILE, "$samplepath/processed_files/fam.$famname.stopgain.nfe" or die;
