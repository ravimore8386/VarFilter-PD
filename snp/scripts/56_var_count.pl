...
open (TXTFILE, "$samplepath/processed_files/res.$famname.out") or die "Cannot open the file $!";

open(my $outfile1, ">","$samplepath/processed_files/res.$famname.dup.out");
...
open(my $outfile, ">","$samplepath/processed_files/res.$famname.out.count");
...
print {$outfile1} "SNV's\tGene\tPhenotype\tHet/Hom\tChr\tStart\tEnd\tRef\tAlt\tFunc.refGene\tGene.refGene\tExonicFunc.refGene\tAAChange.refGene\tcytoBand\tavsnp150\tAF_nfe\tSIFT_pred\tPolyphen2_HDIV_pred\tfamily_number\tSfari\tSpark\tCHD8\tLOUEF\tpLI\tSatterstrom\tRubeis\tautismKB\tEpilepsy\tAllen\tAtlas\tQuickGO\tmapk\tAxon_guidance\twnt\tmRNA_splicing\tRegulation\tPostsycasestic_Density\tPresycasestic_ActiveZone\tPresycasestic\tSycasestic_Vesicles\tGene;Chr;Pos\tFrequencies\n";
# print array string
