...
#USAGE: perl 33_stoploss_gene_ann_cases.pl GF11 /mnt/b2/home4/arc/rm975/wgs/pedigree/GF18/variant_prio/snp/DiagnoVarFilter/dignosis
...
#CHD8.txt
open FILE19, "$samplepath/gene_databases/CHD8.txt" or die "cannot open file";
my %CHD8; while (my $line19=<FILE19>) { chomp($line19); $CHD8{$line19} = ''; }

#LOUEF.txt
open FILE20, "$samplepath/gene_databases/LOUEF.txt" or die "cannot open file";
my %LOUEF; while (my $line20=<FILE20>) { chomp($line20); $LOUEF{$line20} = ''; }

#pLI.txt
open FILE21, "$samplepath/gene_databases/pLI.txt" or die "cannot open file";
my %pLI; while (my $line21=<FILE21>) { chomp($line21); $pLI{$line21} = ''; }

#Sfari.txt
open FILE1, "$samplepath/gene_databases/Sfari.txt" or die "cannot open file";
my %sfari; while (my $line1=<FILE1>) { chomp($line1); $sfari{$line1} = ''; }

#Spark.txt
open FILE2, "$samplepath/gene_databases/Spark.txt" or die "cannot open file";
my %spark; while (my $line2=<FILE2>) { chomp($line2); $spark{$line2} = ''; }

#Satterstrom.txt
open FILE3, "$samplepath/gene_databases/Satterstrom.txt" or die "cannot open file";
my %Satterstrom; while (my $line3=<FILE3>) { chomp($line3); $Satterstrom{$line3} = ''; }

#Rubeis.txt
open FILE4, "$samplepath/gene_databases/Rubeis.txt" or die "cannot open file";
my %Rubeis; while (my $line4=<FILE4>) { chomp($line4); $Rubeis{$line4} = ''; }

#autismKB.txt
open FILE5, "$samplepath/gene_databases/autismKB.txt" or die "cannot open file";
my %autismKB; while (my $line5=<FILE5>) { chomp($line5); $autismKB{$line5} = ''; }

#Epilepsy.txt
open FILE6, "$samplepath/gene_databases/Epilepsy.txt" or die "cannot open file";
my %Epilepsy; while (my $line6=<FILE6>) { chomp($line6); $Epilepsy{$line6} = ''; }

#Allen.txt
open FILE7, "$samplepath/gene_databases/Allen.txt" or die "cannot open file";
my %Allen; while (my $line7=<FILE7>) { chomp($line7); $Allen{$line7} = ''; }

#Atlas.txt
open FILE8, "$samplepath/gene_databases/Atlas.txt" or die "cannot open file";
my %Atlas; while (my $line8=<FILE8>) { chomp($line8); $Atlas{$line8} = ''; }

#QuickGO.txt
open FILE9, "$samplepath/gene_databases/QuickGO.txt" or die "cannot open file";
my %QuickGO; while (my $line9=<FILE9>) { chomp($line9); $QuickGO{$line9} = ''; }

#mapk.txt
open FILE10, "$samplepath/gene_databases/mapk.txt" or die "cannot open file";
my %mapk; while (my $line10=<FILE10>) { chomp($line10); $mapk{$line10} = ''; }

#Axon_guidance.txt
open FILE11, "$samplepath/gene_databases/Axon_guidance.txt" or die "cannot open file";
my %Axon_guidance; while (my $line11=<FILE11>) { chomp($line11); $Axon_guidance{$line11} = ''; }

#wnt.txt
open FILE12, "$samplepath/gene_databases/wnt.txt" or die "cannot open file";
my %wnt; while (my $line12=<FILE12>) { chomp($line12); $wnt{$line12} = ''; }

#mRNA_splicing.txt
open FILE13, "$samplepath/gene_databases/mRNA_splicing.txt" or die "cannot open file";
my %mRNA_splicing; while (my $line13=<FILE13>) { chomp($line13); $mRNA_splicing{$line13} = ''; }

#Regulation.txt
open FILE14, "$samplepath/gene_databases/Regulation.txt" or die "cannot open file";
my %Regulation; while (my $line14=<FILE14>) { chomp($line14); $Regulation{$line14} = ''; }

#Postsynaptic_Density.txt
open FILE15, "$samplepath/gene_databases/Postsynaptic_Density.txt" or die "cannot open file";
my %Postsynaptic_Density; while (my $line15=<FILE15>) { chomp($line15); $Postsynaptic_Density{$line15} = ''; }

#Presynaptic_ActiveZone.txt
open FILE16, "$samplepath/gene_databases/Presynaptic_ActiveZone.txt" or die "cannot open file";
my %Presynaptic_ActiveZone; while (my $line16=<FILE16>) { chomp($line16); $Presynaptic_ActiveZone{$line16} = ''; }

#Presynaptic.txt
open FILE17, "$samplepath/gene_databases/Presynaptic.txt" or die "cannot open file";
my %Presynaptic; while (my $line17=<FILE17>) { chomp($line17); $Presynaptic{$line17} = ''; }

#Synaptic_Vesicles.txt
open FILE18, "$samplepath/gene_databases/Synaptic_Vesicles.txt" or die "cannot open file";
my %Synaptic_Vesicles; while (my $line18=<FILE18>) { chomp($line18); $Synaptic_Vesicles{$line18} = ''; }


open(my $outfile, ">","$samplepath/processed_files/fam.$famname.stoploss.cases.gene.res") or die "cannot open file";
...
open FILE99, "$samplepath/processed_files/fam.$famname.stoploss.cases.out" or die;
