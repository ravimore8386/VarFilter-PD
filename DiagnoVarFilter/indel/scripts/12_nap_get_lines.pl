...
#USAGE: perl 12_nap_get_lines.pl GF15 $samplepath/input_annovar_annotated_txt_files/dignosis

my $famname = $ARGV[0]; #GF15

my $samplepath = $ARGV[1];# "$samplepath/input_annovar_annotated_txt_files/dignosis";

open FILE1, "$samplepath/processed_files/fam.$famname.nap.cnt" or die "cannot open file";
...
open(my $outfile, ">","$samplepath/processed_files/fam.$famname.nap.out");
...
open (TXTFILE, "$samplepath/processed_files/fam.$famname.nap") or die "cannot open file fam_n_com.vpr.dam";  #search file
