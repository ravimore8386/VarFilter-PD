...
#USAGE: perl 47_stopgain_get_bap.pl GF18 /mnt/b2/home4/arc/rm975/wgs/pedigree/GF18/variant_prio/snp/het

my $pedigree = $ARGV[0]; #GF18

my $working_dir = $ARGV[1];# "/mnt/b2/home4/arc/rm975/wgs/pedigree/GF18/variant_prio/snp/het";

open FILE1, "$working_dir/processed_files/ped.$pedigree.stopgain.bap.cnt" or die "cannot open file fam.$pedigree.dam.nfe";

my %hash={};
while (my $line1=<FILE1>) {
   chomp($line1);
   my ($b1,$b2,$b3) = split /:/, $line1;
   $hash{"$b1:$b2"} = "";
}

open(my $outfile, ">","$working_dir/processed_files/ped.$pedigree.stopgain.bap.out");
...
open (TXTFILE, "$working_dir/processed_files/ped.$pedigree.stopgain.bap") or die "cannot open file fam_n_com.vpr.dam";  #search file
