...
#USAGE: perl 8_stoploss_pop.pl GF18 /mnt/b2/home4/arc/rm975/wgs/pedigree/GF18/variant_prio/snp/het

my $pop_freq = 0.01;  # $rest[42] coloum belongs to the nfe non finish europeun population, change if you want to consider any other population frequncies.

my $pedigree = $ARGV[0]; #GF18

my $working_dir = $ARGV[1];# "/mnt/b2/home4/arc/rm975/wgs/pedigree/GF18/variant_prio/snp/het";

open (TXTFILE, "$working_dir/processed_files/ped.$pedigree.stoploss") or die "cannot open file fam.$pedigree.stoploss";#search file
open(my $outfile1, ">","$working_dir/processed_files/ped.$pedigree.stoploss.nfe");
open(my $outfile2, ">","$working_dir/processed_files/ped.$pedigree.stoploss.nfe.cnt");
...
{
chomp;
#   my ($c1,$c2,$c3,$c4,$c5,$c6,$c7,$c8,$c9,$c10,$c11,22$c12,$c13,$c14,$c15,$c16,$c17,$c18,$c19,$c20) = split /\t/, $_;
my (@rest)=split (/\t/);
print $outfile1 "$_\n" if ($_ =~ m/Chr\tStart/);

if ($rest[42] <= $pop_freq)  #nfe non finish europeun pop
{
print $outfile1 "$_\n";
