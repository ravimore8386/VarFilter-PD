...
open (TXTFILE, "$working_dir/processed_files/res.$pedigree.snp.out") or die "Cannot open the file $!";

open(my $outfile1, ">","$working_dir/processed_files/res.$pedigree.snp.list.out");
...
open(my $outfile, ">","$working_dir/processed_files/res.$pedigree.snp.list.count");
...
print {$outfile1} "SNV's\tGene\tPhenotype\tHet/Hom\tChr\tStart\tEnd\tRef\tAlt\tFunc.refGene\tGene.refGene\tExonicFunc.refGene\tAAChange.refGene\tcytoBand\tavsnp150\tAF_nfe\tSIFT_pred\tPolyphen2_HDIV_pred\tfamily_number\tSfari\tSpark\tCHD8\tLOUEF\tpLI\tSatterstrom\tRubeis\tautismKB\tEpilepsy\tAllen\tAtlas\tQuickGO\tmapk\tAxon_guidance\twnt\tmRNA_splicing\tRegulation\tPostsynaptic_Density\tPresynaptic_ActiveZone\tPresynaptic\tSynaptic_Vesicles\tGene;Chr;Pos\tFrequencies\n";
# print array string
foreach my $item (@sorted_uniq_array) {
print {$outfile1} "SNP\t$item\n";
}
