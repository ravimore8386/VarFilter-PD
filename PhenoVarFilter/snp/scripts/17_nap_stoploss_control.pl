...
#USAGE: perl 29_stoploss_control_nap.pl GF18 /mnt/b2/home4/arc/rm975/wgs/pedigree/GF18/variant_prio/snp/het 
...
open FILE1, "$working_dir/input_annovar_annotated_txt_files/$item.snp.txt" or die "cannot open file"; #search contorl file
...
open (TXTFILE2, "$working_dir/input_samples_details/input_bap.txt");
my @bap = <TXTFILE2>;
chomp(@bap);

my $cont2;
foreach my $item2 (@bap) {
open FILE2, "$working_dir/input_annovar_annotated_txt_files/$item2.snp.txt" or die "cannot open file"; #search contorl file
     while (my $line2=<FILE2>) {
     chomp($line2);
     my($c1,$c2) = split /\t/, $line2;
     my $cont2 = "$c1:$c2";
     $hashbap{$cont2} = '';
     }
}

#while ( (my $l) = each %hashbap ) {
#    print $outfile "$l\n";
#}
close(TXTFILE2);
...
#MAP CONTROLS
my %hashmap;
open (TXTFILE1, "$working_dir/input_samples_details/input_map.txt");
my @map = <TXTFILE1>;
chomp(@map);

my $cont1;
foreach my $item1 (@map) {
open FILE1, "$working_dir/input_annovar_annotated_txt_files/$item1.snp.txt" or die "cannot open file"; #search contorl file
     while (my $line1=<FILE1>) {
     chomp($line1);
...
#while ( (my $k) = each %hashmap ) {
#    print $outfile "$k\n";
#}
close(TXTFILE1);

open(my $outfile, ">","$working_dir/processed_files/ped.$pedigree.stoploss.nap.con") or die "cannot open file fam.$pedigree.stoploss.nap.con";
...
open FILE, "$working_dir/processed_files/ped.$pedigree.stoploss.nfe" or die;
