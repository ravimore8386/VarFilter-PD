...
#USAGE: perl 9_nap_indel_control.pl GF18 /mnt/b2/home4/arc/rm975/wgs/pedigree/GF18/variant_prio/indel/het2
...
open FILE, "$samplepath/input_annovar_annotated_txt_files/$item.indel.txt" or die "cannot open file"; #search contorl file
     while (my $line=<FILE>) {
     chomp($line);
     my($a1,$a2) = split /\t/, $line;
     $cont = "$a1:$a2";
...
#MAP CONTROLS
my %hashmap;
open (TXTFILE1, "$samplepath/input_samples_details/input_map.txt");
my @map = <TXTFILE1>;
chomp(@map);

my $cont1;
foreach my $item1 (@map) {
open FILE1, "$samplepath/input_annovar_annotated_txt_files/$item1.indel.txt" or die "cannot open file"; #search contorl file
...
open (TXTFILE2, "$samplepath/input_samples_details/input_bap.txt");
my @bap = <TXTFILE2>;
chomp(@bap);

my $cont2;
foreach my $item2 (@bap) {
open FILE2, "$samplepath/input_annovar_annotated_txt_files/$item2.indel.txt" or die "cannot open file"; #search contorl file
...
open(my $outfile, ">","$samplepath/processed_files/fam.$famname.nap.con") or die "cannot open file";
...
open FILE, "$samplepath/processed_files/fam.$famname.dam.nfe" or die;
